...
namespace Main_Interface.User_Controls.CaiDat_UserControls
{
    public partial class DanhSachChan : UserControl
    {
        ...
        private bool isloadingCalled = false;
        public DanhSachChan()
        {
            InitializeComponent();
        }
        ...
        private async void DanhSachChan_Load(object sender, EventArgs e)
        {
            loading.Show();
        ...
        private async Task loadBlockedUsers()
        {
            ...
            foreach (var meta in allMetas)
            {
                ...
                string myId = meta.userA == Session.LocalId ? meta.userA : meta.userB;
                var blocked = auth.IsBlocked(otherUserId, myId);
                if (!await blocked) continue;
